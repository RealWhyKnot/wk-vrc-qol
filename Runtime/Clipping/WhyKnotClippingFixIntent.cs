...
//
// Per-renderer marker that asks the clipping fixer to rewrite target mesh
// skin weights at play-mode entry and avatar upload. The source mesh asset
// is left untouched; the build hook clones the renderer's current mesh in
// memory, applies the same fix used by the window, then restores the
// original renderer state after the run.
...
namespace WhyKnot.AvatarQol.Components {
    ...
    public sealed class WhyKnotClippingFixIntent : MonoBehaviour, IEditorOnly {
        ...
        [Tooltip("The SkinnedMeshRenderer whose skin weights should be corrected against the comparison surfaces. Defaults to the renderer on this GameObject.")]
        ...
        [Tooltip("Surface distance margin used when detecting triangle intersections, in metres.")]
